...
def test_get_settings():
    ...
    response = client.post("/api/v1/profiles/", json={"@laundrID": "settings_user", "bio": "test bio"})
...
def test_update_settings_unverified():
    # Create a profile first
    response = client.post("/api/v1/profiles/", json={"@laundrID": "unverified_user_settings", "bio": "test bio"})
    user_id = response.json()["user_id"]

    response = client.put(
        f"/api/v1/settings/{user_id}",
        json={"spending_limit": 500}
    )
    assert response.status_code == 403

def test_update_settings_verified():
    # Create a profile first
    response = client.post("/api/v1/profiles/", json={"@laundrID": "verified_user_settings", "bio": "test bio"})
    user_id = response.json()["user_id"]
