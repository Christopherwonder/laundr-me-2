...
def setup_and_teardown():
    ...
    db_profiles.clear()
    db_profiles[1] = FreelancerProfile(user_id=1, laundr_id="@jane_doe", headline="Software Engineer", skill_tags=["Python", "FastAPI"], rating=4.5, reviews=10, sentiment_score=0.9, activity_score=0.85)
    db_profiles[2] = FreelancerProfile(user_id=2, laundr_id="@john_smith", headline="Data Scientist", skill_tags=["Python", "PyTorch"], rating=4.8, reviews=25, sentiment_score=0.95, activity_score=0.9)
    db_profiles[3] = Profile(user_id=3, laundr_id="@sam_jones", headline="Product Manager", skill_tags=["Agile", "Scrum"])
    db_profiles[4] = FreelancerProfile(user_id=4, laundr_id="@emily_white", headline="UX Designer", skill_tags=["Figma", "Sketch"], rating=4.9, reviews=30, sentiment_score=0.98, activity_score=0.92)
    db_profiles[5] = Profile(user_id=5, laundr_id="@chris_green", headline="Python Developer", skill_tags=["Python"])
...
def test_search_no_results():
    headers = {"X-Compliance-Token": "test-token"}
    response = client.post("/api/v1/directory/search", json={"term": "Ruby"}, headers=headers)
    assert response.status_code == 200
    data = response.json()
...
def test_search_empty_term():
    headers = {"X-Compliance-Token": "test-token"}
    response = client.post("/api/v1/directory/search", json={"term": ""}, headers=headers)
    assert response.status_code == 200
    data = response.json()
...
def test_filter_freelancers_no_results():
    headers = {"X-Compliance-Token": "test-token"}
    response = client.post("/api/v1/directory/freelancers/filter", json={"category": "Django"}, headers=headers)
    assert response.status_code == 200
    assert len(response.json()) == 0

def test_filter_freelancers_multiple_criteria():
    headers = {"X-Compliance-Token": "test-token"}
    response = client.post("/api/v1/directory/freelancers/filter", json={"category": "Python", "min_rating": 4.6}, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 1
...
def test_sort_freelancers_by_rating_desc():
    ...
    data = response.json()
    assert len(data) == 3
    assert [d["@laundrID"] for d in data] == ["@emily_white", "@john_smith", "@jane_doe"]

def test_sort_freelancers_by_reviews_asc():
    headers = {"X-Compliance-Token": "test-token"}
    response = client.post("/api/v1/directory/freelancers/sort", json={"sort_by": "reviews"}, headers=headers)
    assert response.status_code == 200
    data = response.json()
    assert len(data) == 3
    assert [d["@laundrID"] for d in data] == ["@jane_doe", "@john_smith", "@emily_white"]
